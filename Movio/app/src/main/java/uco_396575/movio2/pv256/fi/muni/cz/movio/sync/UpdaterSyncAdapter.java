...
import uco_396575.movio2.pv256.fi.muni.cz.movio.db.MovieProvider;
...
public class UpdaterSyncAdapter extends AbstractThreadedSyncAdapter {
    ...
    public void onPerformSync(Account account, Bundle extras, String authority, ContentProviderClient provider, SyncResult syncResult) {
        MovieManager movieProvider = new MovieManager(provider.getLocalContentProvider().getContext());
        List<Movie> movies;
        try {
            MovieClientRetrofitImpl movieClientRetrofit = new MovieClientRetrofitImpl();
            movies = movieClientRetrofit.getBestScifi().execute().body().getMovies();
            movieProvider.updateMovies(movies);
            movies = movieClientRetrofit.getMostPopular().execute().body().getMovies();
            movieProvider.updateMovies(movies);
        } catch (IOException e) {
            e.printStackTrace();
        }
